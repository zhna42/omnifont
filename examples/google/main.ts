import { FontManager, type FontItem, type VariantMetadata, type LoadingState } from '../../src/index';
...
let manager: FontManager | null = null;
let fonts: FontItem[] = [];
let activeFont: FontItem | null = null;

const STORAGE_KEY = 'omnifont.googleApiKey';
apiKeyInput.value = localStorage.getItem(STORAGE_KEY) ?? '';

/** Загружает каталог Google Fonts по API-ключу. */
async function loadCatalog(): Promise<void> {
  const apiKey = apiKeyInput.value.trim();
  if (!apiKey) {
    loadStatus.textContent = 'Введите API-ключ.';
    return;
  }
  ...
  try {
    manager = new FontManager({ apiKey, googleSort: 'popularity', debug: true });
    // Прелоадер управляется событиями загрузки библиотеки.
    ...
    fonts = await manager.init();
    searchInput.disabled = false;
    renderList(fonts);
    loadStatus.textContent = `Семейств: ${fonts.length}`;
  } catch (err) {
    console.error(err);
    loadStatus.textContent = err instanceof Error ? err.message : 'Ошибка загрузки.';
  } finally {
 ...
 * прелоадер), затем применяет CSS. Google использует &text-оптимизацию.
 */
...
/** Скачивает бинарник, логирует данные и применяет через FontFace. */
async function applySelection(): Promise<void> {
  ...
  try {
    ...
    console.group('%cSelectionResult', 'color:#4f8cff;font-weight:bold');
...
// ── Слушатели ──
loadBtn.addEventListener('click', () => void loadCatalog());
